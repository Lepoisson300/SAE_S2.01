...
namespace Unideckbuildduel.Logic.GameData
{
    ...
    public static class LoadData
    {
        ...
        public static Stack<Card> GenStack()
        {
            ...
            for (int i = 0; i < 0; i++) list.Add(new Card { CardType = cardTypes["Dorm"] });
            for (int i = 0; i < 0; i++) list.Add(new Card { CardType = cardTypes["Archive"] });
            for (int i = 0; i < 0; i++) list.Add(new Card { CardType = cardTypes["Cafeteria"] });
            for (int i = 0; i < 30; i++) list.Add(new Card { CardType = cardTypes["Funding"] });
            for (int i = 0; i < 0; i++) list.Add(new Card { CardType = cardTypes["A-List Publication"] });
            for (int i = 0; i < 30; i++) list.Add(new Card { CardType = cardTypes["Faculty Lounge"] });
            for (int i = 0; i < 30; i++) list.Add(new Card { CardType = cardTypes["Research Lab"] });
            for (int i = 0; i < 30; i++) list.Add(new Card { CardType = cardTypes["Official Visit"] });
